...
**※ 現在の実装ではカスタムGLSLシェーダーは使用していません。**

Boxel Editorは、計画時点ではGLSLを用いたカスタムシェーダーでボクセルメッシュの表示と頂点ハイライト機能を実装する予定でした。しかし、現在の実装ではThree.jsの`MeshBasicMaterial`を使用した簡潔な実装となっています。

## 2. 現在の実装方式

### 2.1 使用マテリアル

- **MeshBasicMaterial**: Three.js のビルトインマテリアルを使用
  - ライティングの影響を受けないフラットな色表現
  - DoubleSide で両面描画
  - 動的な色変更に対応（ホバー、選択、着色）

### 2.2 ハイライト表示の実装

- **ホバーハイライト**: 青色（#0099FF）への材質色の動的更新
- **選択ハイライト**: 黄色（#FFFF00）への材質色の動的更新
- **状態保存**: `userData.baseColor` でハイライト前の色を保持
