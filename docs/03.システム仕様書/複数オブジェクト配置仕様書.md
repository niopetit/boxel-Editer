...
Boxel Editorは、メインオブジェクト（編集対象）に加えて、複数の隣接オブジェクト（編集不可）を配置・表示できる。このドキュメントは隣接オブジェクトの配置メカニズムと管理方法を定義する。
...
| ボクセル削除 | ○ | ✕ |
| 面着色 | ○ | ✕ |
| 表示方法 | カラー、フル不透明度 | グレースケール、70-80%不透明度 |
| マウスクリック反応 | ○ | ✕ |
