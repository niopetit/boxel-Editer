...
Boxel Editorのアンドゥ・リドゥシステムは、ユーザーの操作を履歴として記録し、前の状態または次の状態に復元できる。システム要件定義書で定義された以下の操作をサポートする：
...
interface AddVoxelAction extends Action {
  ...
  data: {
    voxelId: string;
    position: Vector3;
    parentFaceId: string;   // 追加元の面
    adjacentDirection: string;  // "up", "down", "left", "right", "front", "back"
  };
}
```
...
#### 3.2.4 カメラ移動（cameraMove）

```typescript
interface CameraMoveAction extends Action {
  type: "cameraMove";
  data: {
...
#### 3.2.5 カメラ回転（cameraRotate）

```typescript
interface CameraRotateAction extends Action {
  type: "cameraRotate";
  data: {
    previousRotation: Quaternion;  // または Euler
...
#### 3.2.6 カメラズーム（cameraZoom）

```typescript
interface CameraZoomAction extends Action {
  type: "cameraZoom";
  data: {
    previousZoom: number;
    newZoom: number;
