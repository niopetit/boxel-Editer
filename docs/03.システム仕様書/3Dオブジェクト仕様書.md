...
- **原点**: (0, 0, 0) = メインオブジェクトの中心
- **配置**: グリッドの中心が原点となるように配置
  - グリッド 32×32 の場合、ボクセルは (-16, 0, -16) ～ (16, 32, 16) に配置
...
```typescript
function generateInitialVoxels(gridX: number, gridY: number): Voxel[] {
  const voxels: Voxel[] = [];
  const offsetX = -gridX / 2;
  const offsetY = 0;  // Y=0を基準
  const offsetZ = -gridY / 2;
